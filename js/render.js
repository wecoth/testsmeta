...
// Рисуется со стороны интерьера: линия с засечками 45° + текст
// ══════════════════════════════════════════════════════════════════
function drawOpeningWidthDimensions() {
  ...
  // Отступы аналогичны drawWallDimensions, но чуть ближе к стене
  // чтобы не накладываться на размеры сегментов
  const LINE_OFF_MM = 120;
  const TEXT_OFF_MM = 230;
  const GAP_MM      = 4;
  ...
  for (const op of appState.openings) {
    ...
    // Позиции краёв проёма вдоль стены
    ...
    // Определяем сторону интерьера — ищем комнату содержащую эту стену
    let sideSign = 1;
    ...
    const label = `${Math.round(opLen)} мм`;
    const color = op.type === 'window' ? '#2563eb' : '#dc2626';
