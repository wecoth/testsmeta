...
import {
  ...
  invalidateJointCache, distanceToWallSurface,
} from '../wall.js';
...
onKeyDown(e) {
  ...
  hitTestObject(wx, wy, screenPoint) {
    ...
    const worldTol = Math.max(10, Math.abs(toWorld(WALL_HIT_PX, 0).x - toWorld(0, 0).x));
    ...
    const op = findClosestOpening(wx, wy, Math.max(80, worldTol * 1.6));
    if (op) return { type: 'opening', id: op.id };

    let bestWall = null;
    let bestDist = worldTol;
    for (const wall of appState.walls) {
      const dist = distanceToWallSurface({ x: wx, y: wy }, wall);
      if (dist < bestDist) {
        bestDist = dist;
        bestWall = wall;
      }
